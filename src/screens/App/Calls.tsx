...
const Calls = () => {
  ...
  console.log('🚀 ~ Calls ~ results:', results);
    ...
    if (recentCallSyncDone) {
      (async () => {
        try {
          ...
          let unique_values = dataForDisplay.reduce((acc, obj) => {
            if (!acc.some(o => o.displayName === obj.displayName)) {
              acc.push(obj);
            }
            return acc;
          }, []);
          originDataRef.current = unique_values ?? [];
  ...
  const onSearch = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        let unique_values = result.reduce((acc, obj) => {
          if (!acc.some(o => o.displayName === obj.displayName)) {
            acc.push(obj);
          }
          return acc;
        ...
        setResults(unique_values);
      }
    },
    [searchOpen, setLoading],
  );

  function returnedFilteredData() {
    const filteredData = results?.filter(
      item => item?.type == selectedFilterIndex,
    );
    console.log(
      '🚀 ~ returnedFilteredData ~ filteredData:',
      filteredData?.length,
    );
  ...
  const handleMessage = useCallback(
    async (_phoneNumber: string, item: any) => {
      try {
  ...
  const renderItem = useCallback(
    ...
    [handleMessage, phoneHandles, isModalVisible, selectedFilterIndex],
  );
    ...
    <View
      ...
      {recentCallSyncDone ? (
        <FlashList
          ...
          data={selectedFilterIndex == 0 ? results : returnedFilteredData()}
